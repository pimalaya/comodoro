...
- Added the `json-schema <DIR>` command, writing one JSON Schema file per command emitting structured output, so a script consuming `--json` can validate what it reads.
  ...
  Three land today, `comodoro-get`, `comodoro-watch` and `comodoro-configure`, mirroring how `manual` writes one page per command. The commands driving the timer report a confirmation rather than data, so they carry none. The schemas come from the types themselves, through a `schemars` cargo feature the `cli` one enables.
